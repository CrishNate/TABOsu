...
SET DLLS_TARGET_LOCATION=%CONFIG_PATH%\bin\x86\Debug
...
set DEBUG_DLLS_LIST=(TABOsu.dll)

for %%i in %DEBUG_DLLS_LIST% do @xcopy "%DLLS_SOURCE_LOCATION%\%%i" "%DLLS_TARGET_LOCATION%" /d /y

set DEBUG_DLLS_LIST=(TABOsu.lib)

for %%i in %DEBUG_DLLS_LIST% do @xcopy "%DLLS_SOURCE_LOCATION%\%%i" "%DLLS_TARGET_LOCATION%" /d /y

set DEBUG_DLLS_LIST=(TABOsu.pdb)

for %%i in %DEBUG_DLLS_LIST% do @xcopy "%DLLS_SOURCE_LOCATION%\%%i" "%DLLS_TARGET_LOCATION%" /d /y


set RELEASE_DLLS_LIST=(TABOsu.dll)

for %%i in %RELEASE_DLLS_LIST% do @xcopy "%DLLS_SOURCE_LOCATION%\%%i" "%DLLS_TARGET_LOCATION%" /d /y

set RELEASE_DLLS_LIST=(TABOsu.lib)

for %%i in %RELEASE_DLLS_LIST% do @xcopy "%DLLS_SOURCE_LOCATION%\%%i" "%DLLS_TARGET_LOCATION%" /d /y

set RELEASE_DLLS_LIST=(TABOsu.pdb)
